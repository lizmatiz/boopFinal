...
namespace main_savitch_14{
                ...
                if(firstNum < 6 && secondNum < 6 && thirdNum < 6 && firstChar < 6 && secondChar < 6 && thirdChar < 6){
                    if((firstNum == secondNum && secondNum == thirdNum)){ // checks to make sure it is valid
                        if((firstChar == secondChar - 1) && (secondChar = thirdChar - 1)){
                            ...
                            next_mover() == COMPUTER)){
                                return true; // if it is valid, it is true
                            }
                        }
                    }
                }
    ...
    void Boop::compute_moves(std::queue<std::string>& moves) const{
        ...
        // loop (or nested loops) that makes original type move strings
            // if move constructed is legal
                // push move into queue
        }
        else if(turnType == 3){
            for(char n1 = '1'; n1 < '7'; n1++){
            ...
            // loop (or nested loops) that make removing 3 move strings, probably longer than the originals
                // if move constructed is legal
                    // push move into queue
        }
        else if(turnType == 1){
            ...
            // loop (or nested loops) that make removing 1 move strings
                // if move constructed is legal
                    // push move into queue
            }
        }
